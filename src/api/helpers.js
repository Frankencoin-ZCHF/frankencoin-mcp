...
export const CG_KEY =
	process.env.COINGECKO_API_KEY || loadKey('.config/coingecko/api_key');
export const CG_BASE = 'https://pro-api.coingecko.com/api/v3';
export const DUNE_KEY =
	process.env.DUNE_API_KEY || loadKey('.config/dune/api_key');
...
export async function apiFetch(path) {
	...
	if (!res.ok)
		throw new Error(`Frankencoin API error ${res.status}: ${path}`);
	return res.json();
}
